...
function MessageDetail() {
    ...
    useEffect(() => {
        axios.get(`${baseURL}/get-messages/${user_id}/${id.id}`).then((res) => {
            console.log(res.data, 'message aanu');
            setMessage(res.data) 
        }).catch((err) => {
            console.log(err)
        })
            ...
            <div className="row g-0">
                ...
                {messages.map((message) => (
                    ...
                    <div className="flex-grow-1 ml-3">
                        {message.sender.id !== user_id && (message.sender_profile.full_name)}
                        {message.sender.id === user_id && (message.receiver_profile.full_name)}
                        <div className="small">
                          {message.message}
                        </div>
                    </div>
                ...
                <div className="position-relative">
                    ...
                    {message.map((message) => (
                        ...
                        {message.sender == user_id && 
                              ...
                              <div>
                              <img
                                  src={`${'http://127.0.0.1:8000'}${message.sender_profile.image}`}
                                  className="rounded-circle mr-1"
                                  alt="Chris Wood"
                                  width={40}
                                  height={40}
